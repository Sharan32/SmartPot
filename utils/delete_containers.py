...
from utils.utils import run_cmd, yes_no_input, docker_cmd
...
def main():
    ...
    container_list = run_cmd(docker_cmd("ps | grep " + docker_config["container_name"] + " | awk '{print $NF}'"))
    ...
    if len(container_list) == 0:
        ...
        if yes_no_input(): # user's answer is "yes"
            for container in container_list:
                ...
                run_cmd(docker_cmd('stop ' + container))
                run_cmd(docker_cmd('rm ' + container))

    # ----- Delete networks -----

    network_list = run_cmd(docker_cmd("network ls | grep " + docker_config["network_name"] + " | awk '{print $2}'"))
    if len(network_list) == 0:
        ...
        if yes_no_input():
            for network in network_list:
                ...
                run_cmd(docker_cmd('network rm ' + network))
